...
public class VoltageMif {
    ...
    public static final String BACKUP = "/data/.hK3475/busMif_stock_voltage";
    ...
    private static String getVoltagePath() {
        ...
        if (devfreqDir.exists() && devfreqDir.isDirectory()) {
            ...
            if (files != null) {
                for (File file : files) {
                    if (file.getName().startsWith("bus_mif") && new File(file, "volt_table").exists()) {
                        return file.getAbsolutePath() + "/volt_table";
                    }
                }
            }
        }
        if (new File("/sys/class/devfreq/bus_mif/volt_table").exists()) {
            return "/sys/class/devfreq/bus_mif/volt_table";
        }
        return "/sys/class/devfreq/soc:bus_mif/volt_table";
    }
